...
loop: it assembles the available agent tools (via a :class:`~agent.ports.ToolProvider`),
...
:class:`~agent.results.ToolCallingTurnResult`.

Accounting/analytics for the turn are the caller's concern (see
:class:`agent.ports.TurnAccounting`); this module emits none itself.
"""
...
from agent.conversation_history import MAX_CONVERSATION_MESSAGES
from agent.ports import ConfirmFn, ErrorReporter, OutputSink, SessionStore, ToolProvider
from agent.prompts import build_action_system_prompt, build_action_user_message
from agent.results import ToolCallingTurnResult
    ...
    *,
    ...
    try:
        ...
        if error_reporter is not None:
            error_reporter.report(exc, context="agent.action_driver", expected=True)
