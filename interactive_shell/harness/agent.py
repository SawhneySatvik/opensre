...
(``agent.engine`` for routing + the conversational assistant, ``agent.driver``
for the action tool-calling turn, ``agent.gather`` for evidence gathering). This
module keeps only what is intrinsically terminal:

* ``handle_message_with_agent`` / ``answer_cli_agent`` — thin shell entry points
  that build the :mod:`agent.ports` adapters and delegate to the engine.
...
from agent.action_plan import ActionPlanAction, parse_action_plan
from agent.context import TurnContext
from agent.results import ShellTurnResult, ToolCallingTurnResult
from interactive_shell.harness.adapters import (
    ShellActionDispatch,
    ShellErrorReporter,
    ShellOutputSink,
...
# ---------------------------------------------------------------------------
# Conversational assistant + turn routing (shell entry -> agent.engine)
# ---------------------------------------------------------------------------
...
def answer_cli_agent(
    ...
    *,
    ...
    Delegates to :func:`agent.engine.answer_cli_agent`, supplying the shell
...
def handle_message_with_agent(
    ...
    *,
    ...
    :func:`agent.engine.run_turn`, which performs the pure path routing.
    """
