"""Interactive-shell adapters implementing the :mod:`agent.ports` Protocols.
...
from agent.prompts import build_environment_block
from interactive_shell.harness.action_dispatch import execute_action_plan
...
class ShellOutputSink:
    """:class:`agent.ports.OutputSink` over a Rich console."""
...
class ShellToolProvider:
    """:class:`agent.ports.ToolProvider` backed by the shell tool registry."""
...
class ShellPromptContextProvider:
    """:class:`agent.ports.PromptContextProvider` over the session grounding caches."""
...
class ShellReasoningClientProvider:
    """:class:`agent.ports.ReasoningClientProvider` for the streaming assistant."""
...
class ShellRunRecordFactory:
    """:class:`agent.ports.RunRecordFactory` producing the shell ``LlmRunInfo``."""
...
class ShellActionDispatch:
    """:class:`agent.ports.ActionDispatch` over the shell action interpreter."""
...
class ShellErrorReporter:
    """:class:`agent.ports.ErrorReporter` over ``report_exception``."""
